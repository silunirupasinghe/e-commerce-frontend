    ...
    : [
        {
          ...
          items: [
            {
              name: "IPS LCD Gaming Monitor with dsfdsfw",
              quantity: 2,
              price: "$100.85",
              total: "$201.70",
            },
...
const EditableHistory: React.FC<{
  history: { status: string; date: string; completed: boolean }[];
  onSave: (
    updatedHistory: { status: string; date: string; completed: boolean }[]
  ) => void;
}> = ({ history, onSave }) => {
                ...
                <Box sx={{ flex: 1 }}>
                  <DetailText
                    sx={{
                      ...
                      color: item.completed ? "#000" : "#757575",
                    }}
                  >
                    {item.status}
                  </DetailText>
...
const OrderDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (order) {
      const updatedOrders = loadOrders().map((o) =>
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Box sx={{ color: "#757575", mx: 1 }}>
            {order.createdAt || order.date}
          </Box>
        </DetailHeader>
        <IconButton
          onClick={() => router.push("/seller/dashboard/orders/all-orders")}
          sx={{ color: "#757575" }}
        >
          <CloseIcon />
        </IconButton>
        ...
        >
          ...
          <Grid sx={{ display: "flex", flexDirection: "row", gap: 2 }}>
            ...
            >
              ...
              <DetailText>
                Address Line: {order.addressLine || "N/A"}
              </DetailText>
              <DetailText>
                Flat / Building: {order.flatBuilding || "N/A"}
              </DetailText>
        ...
        >
          ...
          <Grid>
            ...
            >
              ...
              <DetailText>
                Delivery Fee: {order.deliveryFee || "$0.00"}
              </DetailText>
              <DetailText>
                Platform Fee (5%): {order.platformFee || "$0.00"}
              </DetailText>
              <DetailText>
                Credit Card Charge (2.5%): {order.creditCardCharge || "$0.00"}
              </DetailText>
              <Divider sx={{ my: 1 }} />
              <DetailText sx={{ fontWeight: "bold" }}>
                ...
                {(
                  (parseFloat(order.total.replace(/[^0-9.-]+/g, "") || "0") ||
                    0) +
                  (parseFloat(
                    order.deliveryFee?.replace(/[^0-9.-]+/g, "") || "0"
                  ) || 0) +
                  (parseFloat(
                    order.platformFee?.replace(/[^0-9.-]+/g, "") || "0"
                  ) || 0) +
                  (parseFloat(
                    order.creditCardCharge?.replace(/[^0-9.-]+/g, "") || "0"
                  ) || 0) -
                  (parseFloat(
                    order.discount?.replace(/[^0-9.-]+/g, "") || "0"
                  ) || 0)
                ).toFixed(2)}
              </DetailText>
            </Box>
